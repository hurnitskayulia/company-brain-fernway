---
owner: Lukas
last_reviewed: 2026-09-22
source: Company context pack, sections 1 and 4
---

# T2L (time to live)

Fernway's main onboarding metric: the time from contract signed to first schedule published. See [glossary.md](../glossary.md#t2l).

Typical T2L, per the onboarding flow as actually run: **1–3 days** (see [onboarding/new-customer.md](new-customer.md)).

**Open decision:** Fernway's external pitch is "live in under a day," which doesn't match the 1–3 day typical figure above. Not resolved here — see [decisions.md](../decisions.md#6-t2l-claim-vs-target).

GAP: how T2L is actually measured or tracked (which system, who pulls the number) isn't in the source material.
